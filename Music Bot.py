...
async def on_button_click(interaction):
    global looping,status,player,tc,vc
    if interaction.user.voice!=None:
        if interaction.user.voice.channel==vc:
            if interaction.custom_id=="buttonskip":
                if status[:8]=="Download":
                    status="Cancelling Download"
                    await updateplayer()
                    while status!="Download Cancelled" and status!="Playing Music":
                ...
                voice=bot.voice_clients[0]
                voicechannel=voice.channel
                await interaction.respond(content="Playing next")
                await playnext(voicechannel)
                print("skipping")
            elif interaction.custom_id=="buttonpause":
                while status[:8]=="Download":
                    await asyncio.sleep(0.1)
                voice=bot.voice_clients[0]
                if voice.is_paused():
                    voice.resume()
                    status="Playing Music"
                    await interaction.respond(content="Resume")
                else:
                    voice.pause()
                    status="Paused"
                    await interaction.respond(content="Paused")
                print("Pause/playing")
            elif interaction.custom_id=="buttonstop":
                await interaction.respond(content="Stopping")
                if status[:8]=="Download":
                    status="Cancelling Download"
                    await updateplayer()
                    while status!="Download Cancelled" and status!="Playing Music":
            ...
            elif interaction.custom_id=="buttonloop":
                looping = not looping
                await interaction.respond(content="Toggled Looping")
                print("Looping")
            elif interaction.custom_id=="buttonqueue":
                embed=discord.Embed(title="Music Queue", color=0x0cdae9)
                queue=get_queue()
                s=""
                for i in range(len(queue)):
                    if len(s+"\n"+str(i)+". ["+YouTube(queue[i]).title+"]("+queue[i]+")")>1000:
                        s=s+"\n and "+str(len(queue)-i)+" more"
                        break
                    s=s+"\n"+str(i)+". ["+YouTube(queue[i]).title+"]("+queue[i]+")"
                embed.add_field(name="Queue", value=s)
                await interaction.respond(embed=embed)
                print("Showing Queue")
            elif interaction.custom_id=="buttonrandom":
                await interaction.respond(content="Adding a random song")
                found=False
                while not found:
                    try:
                        found=True
                ...
                interaction.respond(content="This button doesn't work",type=6)
            await updateplayer()
        else:
            await interaction.respond(content="Join the correct VC you muppet")
    else:
        await interaction.respond(content="Join the VC you muppet")
'''
