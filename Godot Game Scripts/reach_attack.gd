...
var behaviour = [[0,0.1,0.2,0.4,0,0.3],
...
func is_valid(tiles) -> bool:
	...
	if not hex_grid.in_los(character.hex_position,tiles[0]) or not hex_grid.in_los(character.hex_position,tiles[1]):
		return false
	if hex_grid.line_distance(character.hex_position,tiles[0])+hex_grid.line_distance(character.hex_position,tiles[1])==3 and tiles[0] in hex_grid.get_surrounding_cells(tiles[1]):
		return true
	else:
		return false
		...
		for j in range(-2,3):
			...
			if V not in [character.hex_position,character.control.attacking]:
				if len(character.control.attacking)==0:
					if hex_grid.in_los(character.hex_position,V) and hex_grid.line_distance(character.hex_position,V)<3:
						tiles.append(V)
...
func get_attacks() -> Array:
	...
	for tile1 in hex_grid.get_surrounding_cells(character.hex_position):
		for tile2 in hex_grid.get_surrounding_cells(tile1):
			if tile2 not in hex_grid.get_surrounding_cells(character.hex_position) and tile2!=character.hex_position and hex_grid.in_los(character.hex_position,tile2) and [tile1,tile2] not in attacks and [tile2,tile1] not in attacks:
